...
export default function DashboardPage() {
  ...
  // para dar contexto de tendência, não só o valor absoluto.
  const saldoDelta = useMemo(() => {
    const currentMonthIdx = now.getMonth()
    const current = monthlyFlow[currentMonthIdx]?.saldo ?? 0
    const previous = monthlyFlow[currentMonthIdx - 1]?.saldo ?? 0
  ...
  // -> vitória) combinado com a comissão ainda não recebida proveniente de
  // licitações ativas/ganhas — é a história financeira do funil em si,
  // não um gráfico genérico.
    ...
    <div className="pb-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 px-6 mt-4">
        <KpiCard
          ...
          elevated
          delta={saldoDelta !== null ? { value: saldoDelta, goodDirection: 'up' } : undefined}
        />
        <KpiCard
      ...
      {/* Elemento de assinatura: Pulso da Operação */}
      {pulseData.editais > 0 && (
        <div className="px-6 mt-4">
          <Card className="p-5">
            <OperationPulse {...pulseData} />
          </Card>
        </div>
      )}
