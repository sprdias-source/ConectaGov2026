...
import { Plus, Search, Pencil, Trash2, ArrowDownCircle, ArrowUpCircle, Check, Receipt } from 'lucide-react'
...
export default function ContasLancamentosTab() {
  ...
  const toggleStatus = (t: Transaction) => {
    ...
    updateTransactionStatus.mutate({ tx: t, newStatus, paymentDate: new Date().toISOString().slice(0, 10) })
  }

  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <Select value={month} onChange={(e) => setMonth(parseInt(e.target.value))} className="w-40">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-5">
        <div className="bg-base-900/60 border border-base-700/50 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-extrabold font-mono text-negative-400">{formatBRL(summary.aPagar)}</p>
              ...
              <tbody>
                {filtered.map((t) => (
                  <tr key={t.id} className="border-b border-base-800/60 hover:bg-base-850/40 transition">
                    ...
                    <td className="px-4 py-3 text-base-200 text-[13px] max-w-[220px] truncate">{t.description}</td>
                    <td className="px-4 py-3 text-base-400 text-[12px] max-w-[140px] truncate">{clientName(t.clientId)}</td>
                    <td className={`px-4 py-3 text-right font-mono font-bold text-[13px] ${t.type === 'Receber' ? 'text-positive-400' : 'text-negative-400'}`}>
