...
import { Plus, Trash2, FileSpreadsheet, Ban, CheckCircle2, Repeat, RepeatIcon } from 'lucide-react'
...
export default function EmpenhosTab() {
  const { empenhos, isLoading, addEmpenho, updateEmpenhoStatus, stopRecurrence, deleteEmpenho } = useEmpenhos()
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-base-400 text-[13px]">Cada empenho é vinculado a uma licitação ganha e gera automaticamente as parcelas de comissão.</p>
        </div>
                  ...
                  <tr key={e.id} className="border-b border-base-800/60 hover:bg-base-850/40 transition">
                    ...
                    <td className="px-4 py-3 text-[12px] text-base-400">
                      <span className="flex items-center gap-1">
                        {e.modoParcelamento === 'recorrente' && e.recorrenciaAtiva && <Repeat className="w-3 h-3 text-accent-400" />}
                        {MODO_LABELS[e.modoParcelamento]}
                        {e.modoParcelamento === 'quantidade_fixa' && ` (${e.quantidadeParcelas}x)`}
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {e.modoParcelamento === 'recorrente' && e.recorrenciaAtiva && e.status !== 'Cancelado' && (
                          <button
                            onClick={() => stopRecurrence.mutate(e)}
                            title="Encerrar recorrência (não gera mais parcelas futuras)"
