...
const TYPES: { value: FinancialAccountType; label: string }[] = [
  ...
  { value: 'INTERNO', label: 'Caixa Interno (controle pessoal)' },
]
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial ? 'Editar Conta' : 'Nova Conta ou Cartão'} maxWidth="max-w-md">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {form.type === 'INTERNO' && (
          <div className="bg-base-800/60 border border-base-700 rounded-lg p-3 text-[12px] text-base-400">
            Esta conta é exclusiva para controle pessoal — os lançamentos vinculados a ela <strong className="text-base-200">não impactam</strong> o Patrimônio, o Dashboard, o Fluxo de Caixa, a Saúde Financeira, nem qualquer outro indicador do sistema. Use para organizar entradas e saídas de caixa interno sem misturar com o financeiro da empresa.
          </div>
        )}
