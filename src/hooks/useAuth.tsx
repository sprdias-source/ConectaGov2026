...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange((event, newSession) => {
      // Quando o usuário clica no link de "esqueci minha senha" recebido
      // por e-mail, o Supabase autentica a sessão temporariamente e
      // dispara este evento específico — usamos isso para mostrar a tela
      // de "defina sua nova senha" em vez de abrir o sistema normalmente
      // com uma sessão que o próprio usuário não iniciou de propósito.
  ...
  const value = useMemo<AuthContextValue>(
    () => ({ user, session, loading, isPasswordRecovery, clearPasswordRecovery, signIn, signUp, signOut }),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [user?.id, session?.access_token, loading, isPasswordRecovery]
  )
