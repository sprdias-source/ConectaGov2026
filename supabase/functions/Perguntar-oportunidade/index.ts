...
async function fetchComRetry(url: string, init: RequestInit, tentativas = 4): Promise<Response> {
  ...
  // resposta mesmo com Mistral configurado e disponível. Erro de rede de
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if ((genRes.status === 429 || genRes.status >= 500) && GEMINI_API_KEY_2) {
      console.warn('[Perguntar-oportunidade] 1ª chave do Gemini indisponível (cota esgotada ou sobrecarga) — tentando 2ª chave (projeto Google Cloud separado)...')
    ...
    // — 3º nível, tenta a mesma pergunta via Mistral Document AI antes de
    ...
    if (genRes.status === 429 || genRes.status >= 500) {
      console.warn('[Perguntar-oportunidade] Gemini indisponível (cota esgotada ou sobrecarga) em todas as chaves configuradas — tentando fallback via Mistral Document AI...')
      try {
