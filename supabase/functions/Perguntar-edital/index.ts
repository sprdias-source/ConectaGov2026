...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if ((genRes.status === 429 || genRes.status >= 500) && GEMINI_API_KEY_2) {
      console.warn('[Perguntar-edital] 1ª chave do Gemini indisponível (cota esgotada ou sobrecarga) — tentando 2ª chave (projeto Google Cloud separado)...')
    ...
    if (genRes.status === 429 || genRes.status >= 500) {
      console.warn('[Perguntar-edital] Gemini indisponível (cota esgotada ou sobrecarga) em todas as chaves configuradas — tentando fallback via Mistral Document AI...')
      try {
