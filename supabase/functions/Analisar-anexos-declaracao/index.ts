...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (genRes.status === 429 && GEMINI_API_KEY_2) {
      console.warn('[Analisar-anexos-declaracao] Cota diária da 1ª chave do Gemini esgotada — tentando 2ª chave (projeto Google Cloud separado)...')
    ...
    if (genRes.status === 429) {
      console.warn('[Analisar-anexos-declaracao] Cota diária do Gemini esgotada em todas as chaves configuradas — tentando fallback via Mistral Document AI...')
      try {
