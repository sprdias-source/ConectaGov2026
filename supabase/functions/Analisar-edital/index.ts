...
async function realizarAnalise(supabase: Supa, edital: Anexo, tr: Anexo | undefined): Promise<{ analise: AnaliseResultado; provedor: string }> {
  ...
  if ((genRes.status === 429 || genRes.status >= 500) && GEMINI_API_KEY_2) {
    console.warn('[Analisar-edital] 1ª chave do Gemini indisponível (cota esgotada ou sobrecarga) — tentando 2ª chave (projeto Google Cloud separado)...')
  ...
  if (genRes.status === 429 || genRes.status >= 500) {
    console.warn('[Analisar-edital] Gemini indisponível (cota esgotada ou sobrecarga) em todas as chaves configuradas — tentando fallback via Mistral Document AI...')
    try {
