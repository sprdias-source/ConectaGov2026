...
// objeto costuma aparecer repetida tanto no Edital quanto no TR — sem ver a
// tabela real, o Mistral processando o Edital sozinho tende a alucinar um
// array CURTO (1-3 itens genéricos derivados só dessa frase) em vez de
// devolver vazio, então checar só "vazio ou não" não bastava: o array
// alucinado não-vazio do Edital vencia por engano a tabela de verdade do
// TR. Uma tabela real dificilmente é mais curta que uma alucinação de
// objeto — por isso a lista mais longa das duas é o sinal mais confiável
// disponível aqui.
...
async function tentarFallbackMistral(supabase: Supa, edital: Anexo, tr: Anexo | undefined, signal: AbortSignal): Promise<AnaliseResultado> {
  ...
  if (resTr.status === 'rejected') {
    console.warn('[Analisar-edital] Mistral não conseguiu processar o Termo de Referência (usando só o Edital):', resTr.reason instanceof Error ? resTr.reason.message : String(resTr.reason))
...
async function processarAnalise(supabase: Supa, analysisRowId: string, edital: Anexo, tr: Anexo | undefined) {
  ...
  try {
    ...
    console.log(`[Analisar-edital] Análise concluída via ${provedor} em ${Date.now() - tInicio}ms total.`)
    ...
    console.error(`Erro ao analisar edital (segundo plano) após ${Date.now() - tInicio}ms:`, mensagem)
