...
async function resolverPasta(supabase: ReturnType<typeof createClient>, accessToken: string, segmentos: string[]): Promise<string> {
  let parentId = 'root'
  let caminhoAtual = ''
...
async function excluirArquivoDrive(accessToken: string, driveFileId: string): Promise<void> {
